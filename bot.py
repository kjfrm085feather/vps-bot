...
async def send_vps_management(ctx_or_interaction, v: dict, requester: discord.User):
    ...
    class ManageButtons(ui.View):
        ...
        async def ssh(self, interaction: discord.Interaction, button: ui.Button):
            ...
            ssh_cmd = None
            # if this VPS was provisioned via docker, try to create a real tmate session inside the container
            docker_name = vrec.get("docker_name") if vrec else None
            if docker_name:
                def _create_tmate():
                    try:
                        # create detached tmate session and then display the ssh connection string
                        subprocess.run(["docker", "exec", docker_name, "bash", "-lc", "tmate -S /tmp/tmate.sock new-session -d"], check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        p = subprocess.run(["docker", "exec", docker_name, "bash", "-lc", "tmate -S /tmp/tmate.sock display -p '#{tmate_ssh}'"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=15)
                        return p.stdout.decode().strip()
                    except Exception:
                        return None

                try:
                    ssh_cmd = await asyncio.to_thread(_create_tmate)
                except Exception:
                    ssh_cmd = None
            ...
            try:
                ...
                if ssh_cmd:
                    emb.add_field(name="SSH Command", value=ssh_cmd, inline=False)
                    emb.add_field(name="Instructions", value="• Copy and paste the SSH command above into your terminal\n• Session expires in 24 hours\n• Do not share this with others", inline=False)
                else:
                ...
                await log_action("SSH Issued", f"SSH session for VPS #{self.vps_id} issued to {user} (cmd sent: {'yes' if ssh_cmd else 'no'})")
                return
            except Exception:
        ...
        async def reinstall(self, interaction: discord.Interaction, button: ui.Button):
            if not self.allowed(interaction.user):
                return await interaction.response.send_message("You cannot use these controls.", ephemeral=True)
            # confirm reinstall (will preserve VPS record, collect docker logs, remove container and re-provision)
            await interaction.response.send_message(f"Confirm reinstall of VPS #{self.vps_id}", view=ReinstallConfirmView(self.vps_id, interaction.user), ephemeral=True)
...
class ReinstallConfirmView(ui.View):
    def __init__(self, vps_id: str, requester: discord.User):
        super().__init__(timeout=60)
        self.vps_id = vps_id
        self.requester = requester
    ...
    async def confirm(self, interaction: discord.Interaction, button: ui.Button):
        ...
        v = VPS_DATA.get("vps", {}).get(vid)
        if not v:
            return await interaction.response.edit_message(content="VPS not found.", embed=None, view=None)

        # gather docker logs if present
        docker_name = v.get("docker_name")
        logs = ""
        if docker_name:
            try:
                p = subprocess.run(["docker", "logs", docker_name], check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                out = (p.stdout.decode() if p.stdout else "")
                err = (p.stderr.decode() if p.stderr else "")
                logs = "== docker logs stdout ==\n" + out + "\n== docker logs stderr ==\n" + err
            except Exception as e:
                logs = f"Failed retrieving docker logs: {e}"

        # send logs to console
        try:
            logging.info(f"Reinstall for VPS #{vid} - docker logs:\n{logs}")
        except Exception:
            pass

        # send logs to configured discord log channel
        try:
            await log_action("Reinstall - Docker Logs", f"VPS #{vid} docker logs:\n{logs}")
        except Exception:
            pass
        ...
        if docker_name:
            try:
                subprocess.run(["docker", "rm", "-f", docker_name], check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            except Exception:
                pass

        # mark as not provisioned and re-provision in background
        try:
            v["provisioned"] = False
            v.pop("docker_name", None)
            v.pop("docker_state", None)
            save_vps_data()
            asyncio.create_task(provision_docker_for_vps(vid))
        except Exception:
            pass

        await interaction.response.edit_message(content=f"Reinstall started for VPS #{vid}. Logs were captured and re-provision underway.", embed=None, view=None)
        await log_action("Reinstall Started", f"Reinstall started for VPS #{vid} by {interaction.user}")
