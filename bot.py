...
async def send_vps_management(ctx_or_interaction, v: dict, requester: discord.User):
    ...
    class ManageButtons(ui.View):
        ...
        @ui.button(emoji="▶️", style=discord.ButtonStyle.success)
        ...
        async def ssh(self, interaction: discord.Interaction, button: ui.Button):
            ...
            ssh_cmd = f"ssh {session}@sfo2.tmate.io"
            ...
            try:
                ...
                emb.add_field(name="Session", value=session, inline=False)
                emb.add_field(name="SSH Command", value=ssh_cmd, inline=False)
                emb.add_field(name="Instructions", value="• Copy and paste the command above\n• Session expires in 24 hours\n• Do not share this link with others", inline=False)
                emb.set_footer(text=f"Vortex Nodes | v{config.get('version','1.0.0')} Today At {ts}")
                await dm.send(embed=emb)
                await log_action("SSH Issued", f"SSH session for VPS #{self.vps_id} issued to {user}")
                return
            except Exception:
...
async def on_message(message: discord.Message):
    ...
    # Award 5 credits per message with a 5-second cooldown to prevent spamming
    try:
        uid = str(message.author.id)
        now = int(datetime.now(timezone.utc).timestamp())
        last = MESSAGE_COOLDOWNS.get(uid, 0)
        if now - last >= 5:
            ensure_user_record(uid)
            USER_DB["users"][uid]["credits"] = USER_DB["users"][uid].get("credits", 0) + 5
            MESSAGE_COOLDOWNS[uid] = now
            try:
                save_user_db()
            except Exception:
                pass
...
async def provision_docker_for_vps(vid: str):
    ...
    try:
        ...
        if ok:
            ...
            v["docker_error"] = err
            save_vps_data()
            await log_action("Docker Provision Error", f"Failed to provision VPS #{vid}: {err}")
    except Exception as e:
        try:
            v = VPS_DATA.get("vps", {}).get(vid)
            if v:
...
async def purge_start(ctx):
    ...
    purge["active"] = True
    save_vps_data()
    await ctx.send("Purge started. Deleting unprotected VPS...")
    await log_action("Purge Started", f"Purge started by {ctx.author}")
    ...
    purge["active"] = False
    save_vps_data()
    await ctx.send(f"Purge complete. Removed: {', '.join(removed) if removed else 'none'}")
